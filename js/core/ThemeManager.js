...
var ThemeManager = {
  ...
  _registerBuiltinThemes: function() {
    ...
    // Additional themes omitted for brevity (keep existing values)
    this.register('ocean', { name: { en: 'Ocean', ar: 'محيط' }, vars: { '--bg': '#0a1628', '--bg2': '#0f1f35', '--bg3': '#142842', '--surface': '#1a3050', '--surface2': '#1f3a5a', '--border': '#2a4a6a', '--border2': '#3a5a7a', '--text': '#e0f0ff', '--text2': '#80b0d0', '--text3': '#5090b0', '--accent': '#00aaff', '--accent2': '#0088dd', '--accent-bg': 'rgba(0,170,255,0.12)', '--accent-border': 'rgba(0,170,255,0.4)', '--success': '#00cc88', '--success-bg': 'rgba(0,204,136,0.12)', '--danger': '#ff6666', '--danger-bg': 'rgba(255,102,102,0.12)', '--warning': '#ffaa00', '--warning-bg': 'rgba(255,170,0,0.12)', '--tmA': '#00aaff', '--tmA-bg': 'rgba(0,170,255,0.15)', '--tmB': '#ff6666', '--tmB-bg': 'rgba(255,102,102,0.15)', '--tmAgk': '#00cc88', '--tmBgk': '#ff99cc', '--shadow': '0 8px 24px rgba(0,0,0,0.5)', '--shadow-sm': '0 1px 3px rgba(0,0,0,0.4)', '--bg-secondary': '#0f1f35', '--bg-tertiary': '#142842', '--text-secondary': '#80b0d0', '--accent-dim': 'rgba(0,170,255,0.15)', '--accent-hover': '#0088dd' } });
    this.register('pink', { name: { en: 'Pink', ar: 'وردي' }, vars: { '--bg': '#1a0a14', '--bg2': '#251018', '--bg3': '#2e1520', '--surface': '#3a1a28', '--surface2': '#442030', '--border': '#5a2a40', '--border2': '#6a3a50', '--text': '#ffe0f0', '--text2': '#d0a0c0', '--text3': '#b080a0', '--accent': '#ff66aa', '--accent2': '#ee4488', '--accent-bg': 'rgba(255,102,170,0.12)', '--accent-border': 'rgba(255,102,170,0.4)', '--success': '#66ddaa', '--success-bg': 'rgba(102,221,170,0.12)', '--danger': '#ff6688', '--danger-bg': 'rgba(255,102,136,0.12)', '--warning': '#ffaa66', '--warning-bg': 'rgba(255,170,102,0.12)', '--tmA': '#ff66aa', '--tmA-bg': 'rgba(255,102,170,0.15)', '--tmB': '#ff6688', '--tmB-bg': 'rgba(255,102,136,0.15)', '--tmAgk': '#66ddaa', '--tmBgk': '#ffaa66', '--shadow': '0 8px 24px rgba(0,0,0,0.5)', '--shadow-sm': '0 1px 3px rgba(0,0,0,0.4)', '--bg-secondary': '#251018', '--bg-tertiary': '#2e1520', '--text-secondary': '#d0a0c0', '--accent-dim': 'rgba(255,102,170,0.15)', '--accent-hover': '#ee4488' } });
    this.register('purple', { name: { en: 'Purple', ar: 'بنفسجي' }, vars: { '--bg': '#140a1a', '--bg2': '#1c1022', '--bg3': '#22152a', '--surface': '#2a1a35', '--surface2': '#32203e', '--border': '#422a50', '--border2': '#523a60', '--text': '#f0e0ff', '--text2': '#c0a0d0', '--text3': '#a080b0', '--accent': '#aa66ff', '--accent2': '#8844ee', '--accent-bg': 'rgba(170,102,255,0.12)', '--accent-border': 'rgba(170,102,255,0.4)', '--success': '#66ddaa', '--success-bg': 'rgba(102,221,170,0.12)', '--danger': '#ff6688', '--danger-bg': 'rgba(255,102,136,0.12)', '--warning': '#ffaa66', '--warning-bg': 'rgba(255,170,102,0.12)', '--tmA': '#aa66ff', '--tmA-bg': 'rgba(170,102,255,0.15)', '--tmB': '#ff6688', '--tmB-bg': 'rgba(255,102,136,0.15)', '--tmAgk': '#66ddaa', '--tmBgk': '#ffaa66', '--shadow': '0 8px 24px rgba(0,0,0,0.5)', '--shadow-sm': '0 1px 3px rgba(0,0,0,0.4)', '--bg-secondary': '#1c1022', '--bg-tertiary': '#22152a', '--text-secondary': '#c0a0d0', '--accent-dim': 'rgba(170,102,255,0.15)', '--accent-hover': '#8844ee' } });
    this.register('forest', { name: { en: 'Forest', ar: 'غابة' }, vars: { '--bg': '#0a1a0a', '--bg2': '#0f220f', '--bg3': '#142a14', '--surface': '#1a351a', '--surface2': '#203e20', '--border': '#2a502a', '--border2': '#3a603a', '--text': '#e0ffe0', '--text2': '#a0d0a0', '--text3': '#80b080', '--accent': '#44cc44', '--accent2': '#33aa33', '--accent-bg': 'rgba(68,204,68,0.12)', '--accent-border': 'rgba(68,204,68,0.4)', '--success': '#66dd66', '--success-bg': 'rgba(102,221,102,0.12)', '--danger': '#ff6666', '--danger-bg': 'rgba(255,102,102,0.12)', '--warning': '#ffaa00', '--warning-bg': 'rgba(255,170,0,0.12)', '--tmA': '#44cc44', '--tmA-bg': 'rgba(68,204,68,0.15)', '--tmB': '#ff6666', '--tmB-bg': 'rgba(255,102,102,0.15)', '--tmAgk': '#66dd66', '--tmBgk': '#ffaa66', '--shadow': '0 8px 24px rgba(0,0,0,0.5)', '--shadow-sm': '0 1px 3px rgba(0,0,0,0.4)', '--bg-secondary': '#0f220f', '--bg-tertiary': '#142a14', '--text-secondary': '#a0d0a0', '--accent-dim': 'rgba(68,204,68,0.15)', '--accent-hover': '#33aa33' } });
  },
  ...
  apply: function(themeId) {
    ...
    try { if (typeof theme !== 'undefined') theme = themeId; } catch(e) {}
    var vars = this._themes[themeId].vars || {};
    var el = document.documentElement;
    try { el.setAttribute('data-theme', themeId); el.dataset.theme = themeId; } catch(e) {}
    try {
      for (var key in vars) {
  ...
  applyCanvasTheme: function(themeId) {
    ...
    if (cc) {
      try { cc.style.background = ct.bg; cc.setAttribute('data-canvas-theme', themeId); } catch (e) {}
    }
    // Update canvas grid color via CSS variable
    try { document.documentElement.style.setProperty('--canvas-grid', ct.grid); } catch (e) {}
  ...
  list: function() {
    var r = [];
    for (var id in this._themes) if (Object.prototype.hasOwnProperty.call(this._themes, id)) r.push({ id: id, name: this._themes[id].name });
    return r;
  },

  listCanvas: function() {
    var r = [];
    for (var id in this._canvasThemes) if (Object.prototype.hasOwnProperty.call(this._canvasThemes, id)) r.push({ id: id, name: this._canvasThemes[id].name });
    return r;
  },

  // Cycle themes if needed — returns the current theme id
  toggle: function() {
    var keys = Object.keys(this._themes);
    if (!keys.length) return this._current;
    var idx = keys.indexOf(this._current);
    var next = (idx < 0 || idx === keys.length - 1) ? 0 : idx + 1;
  ...
  getVar: function(v) { try { return getComputedStyle(document.documentElement).getPropertyValue(v).trim(); } catch (e) { return ''; } },
